            ...
            while temp_sequence:
                ...
                while now_budget < total_budget and mep_i_node != '-1':
                    ...
                    if round(now_budget + sc, 4) >= total_budget and bud_iter and not temp_sequence:
                        ...
                        temp_sequence.append([round(total_cost / (2 ** bud_iter.pop(0)), 4), now_budget, now_profit, copy.deepcopy(seed_set), temp_celf_heap, ss_time])
                ...
                ss_time_sequence[len(bud_iter)].append(ss_time)
                seed_set_sequence[len(bud_iter)].append(seed_set)

        eva_model = EvaluationM(self.model_name, self.dataset_name, self.product_name, self.cascade_model)
        for bi in range(len(self.budget_iteration)):
            for wallet_distribution_type in self.wd_seq:
                for ppp in self.ppp_seq:
                    eva_model.evaluate(bi, wallet_distribution_type, ppp, seed_set_sequence[bi], ss_time_sequence[bi])
            ...
            while temp_sequence:
                ...
                while now_budget < total_budget and mep_i_node != '-1':
                    ...
                    if round(now_budget + sc, 4) >= total_budget and bud_iter and not temp_sequence:
                        ...
                        temp_celf_heap = copy.deepcopy(celf_heap)
                        heap.heappush_max(temp_celf_heap, mep_item)
                        temp_sequence.append([round(total_cost / (2 ** bud_iter.pop(0)), 4), now_budget, now_profit, copy.deepcopy(seed_set),
                                              copy.deepcopy(expected_profit_k), copy.deepcopy(now_seed_forest), temp_celf_heap, mep.copy(), ss_time])
                    ...
                    if mep_flag == seed_set_length:
                        ...
                        while len(mep_item_sequence) <= self.batch and celf_heap[0][3] != seed_set_length and celf_heap[0][2] != '-1':
                            mep_item = heap.heappop_max(celf_heap)
                            mep_ratio, mep_k_prod, mep_i_node, mep_flag = mep_item
                            if round(now_budget + seed_cost_dict[mep_i_node], 4) <= total_budget:
            ...
            while temp_sequence:
                ...
                while now_budget < total_budget and mep_i_node != '-1':
                    ...
                    if round(now_budget + sc, 4) >= total_budget and bud_iter and not temp_sequence:
                        ...
                        temp_sequence.append([round(total_cost / (2 ** bud_iter.pop(0)), 4), now_budget, copy.deepcopy(seed_set), temp_degree_heap, ss_time])
        ...
        for sample_count in range(self.sample_number):
            ...
            for k in range(num_product):
                ...
                while temp_sequence:
                    ...
                    ss_time = round(time.time() - ss_start_time + ss_acc_time, 4)
                    print('ss_time = ' + str(ss_time)) + 'sec, cost = ' + str(now_budget) + ', seed_set_length = ' + str([len(s_set_k) for s_set_k in seed_set])
                    ss_time_sequence[len(bud_iter)][sample_count] += ss_time
            ...
            for bi in self.budget_iteration:
                ...
                ss_time_sequence[bi][sample_count] += ss_time
                seed_set = mep_result[1]
                seed_set_sequence[sample_count].append(seed_set)
            ...
            while temp_sequence:
                ...
                while now_budget < total_budget and mep_i_node != '-1':
                    ...
                    if round(now_budget + sc, 4) >= total_budget and bud_iter and not temp_sequence:
                        ...
                        temp_sequence.append([round(total_cost / (2 ** bud_iter.pop(0)), 4), now_budget, copy.deepcopy(seed_set), temp_random_node_set, ss_time])
...
class ModelPW:
    ...
    def model_ngpw(self):
        ...
        for bi in range(len(self.budget_iteration)):
            for ppp in self.ppp_seq:
                eva_model.evaluate(bi, self.wallet_distribution_type, ppp, seed_set_sequence[bi], ss_time_sequence[bi])
